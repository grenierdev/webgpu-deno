...
const textureA = new Uint8Array(img.data);

const gpuTextureA = device.createTexture({
	size: {
		width: img.width,
		height: img.height,
	},
...
const textureASampler = device.createSampler({
...
const shaderModule = device.createShaderModule({
	code: `
		...
		fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
			return textureSample(Texture, Sampler, in.TexCoord);
		}
	`
});

const textureABindGroupLayout = device.createBindGroupLayout({
	entries: [
		{
			binding: 0,
...
const textureABindGroup = device.createBindGroup({
	layout: textureABindGroupLayout,
	entries: [
		{
			binding: 0,
			resource: gpuTextureA.createView()
		},
		{
			binding: 1,
			resource: textureASampler
		}
	]
});

const pipelineLayout = device.createPipelineLayout({
	bindGroupLayouts: [textureABindGroupLayout],
});
...
// Render pass
{
	...
	passEncoder.setBindGroup(0, textureABindGroup);
...
Deno.writeFileSync("./hello-triangle-tex.png", image);
